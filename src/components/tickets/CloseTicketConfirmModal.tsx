...
type CloseTicketConfirmModalProps = {
  isOpen: boolean;
  ticketTitle?: string;
  isPending?: boolean;
  error?: string | null;
...
export default function CloseTicketConfirmModal({
  isOpen,
  ticketTitle,
  isPending = false,
  error,
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="close-ticket-modal-title"
    >
      <button
        type="button"
        aria-label="بستن"
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-[2rem] border border-gray-100 bg-white shadow-2xl animate-in zoom-in-95 dark:border-white/10 dark:bg-[#1c1e26]">
        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-white/10">
          <div className="flex items-center gap-2">
            <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-gray-500/10 text-gray-500">
              <XCircle className="h-4 w-4" />
            </span>
            <h2 id="close-ticket-modal-title" className="text-base font-black text-gray-900 dark:text-white">
              بستن تیکت
            </h2>
          </div>
          <button
        ...
        <div className="space-y-4 px-5 py-6">
          <p className="text-sm font-medium leading-relaxed text-gray-600 dark:text-gray-300">
            آیا از بستن این تیکت مطمئن هستید؟ پس از بستن، امکان ارسال پیام جدید روی این تیکت وجود نخواهد داشت.
          </p>

          {ticketTitle ? (
            <div className="rounded-2xl border border-gray-100 bg-gray-50 px-4 py-3 dark:border-white/10 dark:bg-white/5">
              <p className="text-[11px] font-bold text-gray-400">عنوان تیکت</p>
              <p className="mt-1 text-sm font-black text-gray-900 dark:text-white">{ticketTitle}</p>
            </div>
          ) : null}
        ...
        <div className="flex flex-col-reverse gap-3 border-t border-gray-100 px-5 py-4 sm:flex-row sm:justify-end dark:border-white/10">
          ...
          <button
            ...
            disabled={isPending}
            className={cn(
              "flex cursor-pointer items-center justify-center gap-2 rounded-2xl bg-gray-600 px-5 py-3 text-sm font-black text-white shadow-lg shadow-gray-600/20 transition-all hover:bg-gray-700 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60 dark:bg-gray-500 dark:hover:bg-gray-400"
            )}
          >
            {isPending ? (
              <>
                <span className="h-4 w-4 animate-spin rounded-full border-2 border-white/30 border-t-white" />
                <span>در حال بستن...</span>
              </>
            ) : (
              <span>بله، بستن تیکت</span>
            )}
          </button>
        </div>
      </div>
