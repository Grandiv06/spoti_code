...
export default function ThemeSettingsPanel({
  description = "حالت نمایش پنل را انتخاب کنید. تغییرات بلافاصله اعمال می‌شود.",
  infoText = "پوسته انتخابی شما به‌صورت خودکار ذخیره می‌شود و در تمام بخش‌های آکادمی اعمال خواهد شد.",
  ...
  return (
    <div className="relative overflow-hidden rounded-[1.75rem] border border-gray-200/80 bg-white p-6 shadow-[0_20px_60px_-24px_rgba(15,23,42,0.18)] dark:border-white/10 dark:bg-[#171922] dark:shadow-[0_20px_60px_-24px_rgba(0,0,0,0.55)] md:p-7">
        ...
        {themeOptions.map((option) => {
          ...
          return (
            <button
              ...
              className={cn(
                "group relative flex cursor-pointer flex-col overflow-hidden rounded-[1.35rem] border text-right transition-all duration-300",
                isActive
              ...
              <div className="flex items-center justify-between gap-3 p-4 pt-3">
                ...
                <div
                  className={cn(
                    ...
                    isActive
                      ? "scale-100 border-primary bg-primary text-white opacity-100"
                      : "scale-90 border-gray-300 bg-white opacity-0 group-hover:opacity-40 dark:border-white/20 dark:bg-white/5"
                  )}
                >
      ...
      <div className="relative mt-6 flex items-center gap-2.5 rounded-2xl border border-dashed border-gray-200 bg-gray-50/70 px-4 py-3 dark:border-white/10 dark:bg-white/[0.03]">
        ...
        <p className="text-xs leading-relaxed text-gray-500 dark:text-gray-400">{infoText}</p>
      </div>
    </div>
  );
}
