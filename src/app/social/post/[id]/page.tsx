...
import { 
  ArrowRight, Github, ExternalLink, Calendar, Heart, Bookmark, Share2, 
  Layers, Code2, AlertCircle, Lightbulb, Rocket, Wrench, CheckCircle2, Clock
} from 'lucide-react';
...
export default function PostDetailPage(props: PageProps) {
  ...
  const { getPostById, currentUser, likePost, bookmarkPost, followUser } = useSocial();
  const rawPost = getPostById(params.id);

  // Mock extended data since backend doesn't exist yet
  const post: SocialPost | undefined = rawPost ? {
      ...rawPost,
      status: 'COMPLETED',
      difficulty: 'INTERMEDIATE',
      extendedDescription: {
          idea: "این پروژه با هدف ساده‌سازی فرآیند یادگیری برنامه‌نویسی برای فارسی‌زبانان ایجاد شده است. با توجه به کمبود منابع تعاملی، تصمیم گرفتم پلتفرمی بسازم که یادگیری را جذاب‌تر کند.",
          challenges: "یکی از بزرگترین چالش‌ها، پیاده‌سازی سیستم کامپایلر آنلاین بود که نیاز به مدیریت دقیق منابع سرور و امنیت بالا داشت. همچنین طراحی رابط کاربری که هم ساده و هم کاربردی باشد زمان زیادی برد.",
          learned: "در این پروژه کار با WebAssembly و Docker را به صورت عمیق یاد گرفتم. همچنین تجربه زیادی در زمینه بهینه‌سازی پرفورمنس Next.js کسب کردم.",
          future: "قصد دارم در آینده بخش منتورینگ و امکان برگزاری مسابقات کدنویسی را نیز به پلتفرم اضافه کنم."
      },
      // Ensure comments count is consistent if we mock comments below
  } : undefined;
  ...
  if (!post) {
      return (
          <div className="flex flex-col items-center justify-center min-h-[50vh] text-center">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">پروژه یافت نشد</h2>
  ...
  const allImages = post.coverImageUrl ? [post.coverImageUrl, ...post.galleryImageUrls] : post.galleryImageUrls;
  ...
  const COMMENTS: SocialComment[] = Array.from({ length: 3 }).map((_, i) => ({
      ...
      content: i === 0 
        ? "پروژه بسیار تمیز و حرفه‌ای پیاده‌سازی شده. داکیومنت‌ها هم عالی بودن."
        : "تبریک میگم، طراحی UI خیلی چشم‌نوازه.",
      likesCount: i * 2 + 5,
  ...
  const relatedProjects: SocialPost[] = []; // Mock empty for now or fetch from context if available
            ...
            <div className="lg:col-span-8 space-y-12">
                ...
                <section className="space-y-6">
                    <div className="flex flex-wrap gap-3">
                        {post.status && (
                            <Badge 
                                variant={post.status === 'COMPLETED' ? 'success' : post.status === 'IN_PROGRESS' ? 'warning' : 'outline'}
                                className="px-3 py-1"
                            >
                                {post.status === 'COMPLETED' ? 'تکمیل شده' : post.status === 'IN_PROGRESS' ? 'در حال توسعه' : 'نیاز به بازخورد'}
                            </Badge>
                        )}
                        {post.difficulty && (
                            <Badge variant="glass" className="px-3 py-1">
                                {post.difficulty === 'BEGINNER' ? 'مقدماتی' : post.difficulty === 'INTERMEDIATE' ? 'متوسط' : 'پیشرفته'}
                            </Badge>
                        )}
                ...
                <section>
                 ...
                 {/* 7. Related Projects (Desktop in flow) */}
             ...
             <div className="lg:col-span-4 space-y-8 h-fit lg:sticky lg:top-24">
                <AuthorCard 
                    author={post.author} 
                    isFollowing={false} // Would come from user context check
